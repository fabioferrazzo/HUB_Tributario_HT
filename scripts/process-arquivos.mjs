...
function detectTools() {
  return {
    ...
    ocrmypdf: firstExistingCommand([process.env.OCRMYPDF_BIN, "ocrmypdf"]) || detectPythonModuleCommand("ocrmypdf"),
...
function runCommand(command, args) {
  ...
  print(`Executando: ${command.label} ${commandArgs.map(quoteArg).join(" ")}`);
  const result = spawnSync(command.value, commandArgs, {
    encoding: "utf8",
    stdio: "pipe",
    shell: false
  });
...
function toCommand(value) {
  ...
  if (/^py\s+-m\s+ocrmypdf$/i.test(commandValue)) {
    return {
      value: "py",
      argsPrefix: ["-m", "ocrmypdf"],
      label: "py"
    };
  }
  if (/^python\s+-m\s+ocrmypdf$/i.test(commandValue)) {
    return {
      value: "python",
      argsPrefix: ["-m", "ocrmypdf"],
      label: "python"
  ...
  return {
    value: commandValue,
    argsPrefix: [],
    label: commandValue.includes("\\") || commandValue.includes("/") ? `"${commandValue}"` : commandValue
  };
}
